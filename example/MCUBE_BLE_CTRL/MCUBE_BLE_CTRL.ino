...
uint8_t cmd[MAX_CMDLEN]; // received bytes
unsigned char toBTCommand[MAX_CMDLEN];
unsigned char i = 0;
...
void commandRCV()
{
	//Serial.println("commandRCV");
	
	while (Serial1.available() > 0)
	{
		uint8_t ucRcvByte = Serial1.read();
		
		if (false)
		{	
			Serial.print("Rcv Byte: ");
			if(rcvIndex == 0)
				Serial.print("0x"); 
			...
			Serial.print(ucRcvByte, HEX);
			Serial.print(" ");
			Serial.print("rcvIndex: ");
			Serial.println(rcvIndex);
		}
		
		if (ucRcvByte != '\n')
		{
			Serial.println();
			if(rcvIndex < MAX_CMDLEN)
			{
				if (rcvIndex == 0)
				{	
...
void loop()
{
	...
	if(Serial.available() > 0)
	{
		...
		if (inByte != '\r' && inByte != '\n')
		{
			...
			//if (isDebug) ardprintf("inByte= %c\n", inByte );

			if (i < (MAX_CMDLEN - 4))
			{
				cmd[(i) % (MAX_CMDLEN - 4)] = (uint8_t)Ascii2Hex(inByte);
				if (isDebug)
				{
					Serial.print("i=");
					Serial.println(i);
				...
				if (i == 1)
				{
					...
					Serial.println();
					//Serial.println(byte(trans_msg_t.CMD_HEADER), HEX);
				
					if(((trans_msg_t.CMD_HEADER == PEDOMETER_NOTIFICATION_ENABLE) ||  
						 (trans_msg_t.CMD_HEADER == SLEEP_MONITOR_NOTIFICATION_ENABLE)) && 
						 (notifyMode == false))
					{
						notifyMode = true;
						Serial.println("notifyMode: false -> true");
					}
					else if ( notifyMode == true)
					{
						notifyMode = false;
						Serial.println("notifyMode: true -> false");
					}
				}
				/* Control or Response*/
				else if (i == 3)
				{
					...
					Serial.println();
          //Serial.println(byte(trans_msg_t.CTRL_RESPONSE), HEX);
				}
				/*Payload length*/
				else if (i == 5)
				{
          ...
          Serial.println();    
					//Serial.println(byte(trans_msg_t.PAYLOAD_LEN), HEX);
				}
				/*Payload*/
				else if (i >= 6 && i <= MAX_CMDLEN)
				{
					if (trans_msg_t.PAYLOAD_LEN > 0)
					{
						if ((i <= (6 + trans_msg_t.PAYLOAD_LEN * 2)) && (i % 2 == 1))
						{
							if (isDebug) Serial.print("payLoadIndex=");
							if (isDebug) Serial.println(payLoadIndex);
						...
						else
						{
							if (payLoadIndex == trans_msg_t.PAYLOAD_LEN)
							{
								...
								if (isDebug) 
								{
									Serial.print("cmdEndIndex= ");
									Serial.println(cmdEndIndex);
								}
								payLoadIndex++;
							}
						}
		...
		else if (inByte == '\r')
		{
			...
			cmd[MAX_CMDLEN-4] = (uint8_t) 0;
			cmd[MAX_CMDLEN-3] = (uint8_t) 13;
		}
		/* New Line Byte */
		if (inByte == '\n')
		{
			//Serial.println("New Line");
			cmd[MAX_CMDLEN-2] = (uint8_t) 0;
			cmd[MAX_CMDLEN-1] = (uint8_t) 10;
			i = i + 2;
			if (isDebug) 
			{	
				Serial.print("i=");
			...
			else
			{
				...
				if(isDebug)
				{
					for (int y = 0; y < cmdEndIndex; y++)
					{
						...
						if (toBTCommand[y] < 0x10) 
						{
							Serial.print("0");
						} 
					    Serial.println(byte(toBTCommand[y]), HEX);
					}
				}
				delayMicroseconds(10);
				ble.commandTRANS(trans_msg_t.CMD_HEADER, &toBTCommand[0], cmdEndIndex);
			}
			cmdString = "";
			payLoadIndex = 0;
			isEnd = false;
