...
#define cmd_payload_sleepmonitor_sensitivity_low                    0
#define cmd_payload_sleepmonitor_sensitivity_medium                 1
#define cmd_payload_sleepmonitor_sensitivity_high                   2
...
typedef enum
{
    ...
    SLEEP_MONITOR_SENSITIVITY_SET               =0b00100010, //0x22   
...
public:
  uint8_t inputCmd;
  void commandTRANS(uint8_t bCmdHeader, uint8_t* baCmdBytes, uint8_t len);
  void init(void);
  bool commandRCV(void);
  bool checkAscii(uint8_t thisChar);
  void printHeaderCmd(uint8_t cmdBytes);
  void PrintHex(uint8_t data);
  void PrintHexByteArray(uint8_t *data, uint8_t length);
};
